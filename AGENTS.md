...
- `tox.ini` defines per-module test environments (e.g. `-loggers`, `-flask_app`, `-sql_migrate`, `-http_apis`, `-plugins`, `-flask_plugins`, `-auth`) that install only that module's extras before running its test file — prefer the matching tox env when testing a single module in isolation.
