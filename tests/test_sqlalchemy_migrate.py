...
def sql_migrate_object():
    ...
    rmtree(os.path.join(os.path.dirname(os.path.realpath(__file__)), "applications", "migrations"))
...
class TestSqlMigrator:
    ...
    def test_migration_run(self, sql_migrate_object):
        ...
        assert os.path.exists(os.path.join(sql_migrate_object.current_dir, "migrations"))

        sql_migrate_object.db_migrate()

        assert len(os.listdir(os.path.join(sql_migrate_object.current_dir, "migrations", "versions"))) != 0
